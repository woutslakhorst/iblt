...
// NewFilter256 creates a bloom filter with 1024 bytes.
//      re-calc -->     This is enough for a data set of ~656 with a FPR of around 0.001 using 5 hash functions.
...
func (f *Filter) Add(data []byte) bool {
	...
	if eq(and([]byte{}, f.fingerprint, dataFingerprint), dataFingerprint) {
		return false
	}

	f.fingerprint = or([]byte{}, f.fingerprint, dataFingerprint)
...
func (f *Filter) Check(data []byte) bool {
	dataFingerprint := f.hashValues(data)
	return eq(and([]byte{}, f.fingerprint, dataFingerprint), dataFingerprint)
}
...
//
func and(r []byte, fp []byte, dfp []byte) []byte {
	r = append(r, fp[0] & dfp[0])

	if len(fp) == 1 {
		return r
	}

	return and(r, fp[1:], dfp[1:])
}

func or(r []byte, fp []byte, dfp []byte) []byte {
	r = append(r, fp[0] | dfp[0])
	...
	return or(r, fp[1:], dfp[1:])
}

func eq(fp []byte, dfp []byte) bool {
	if fp[0] != dfp[0] {
		return false
	}
